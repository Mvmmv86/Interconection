...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "source",
            postgresql.ENUM("SCANNER", "MANUAL", "STATIC", name=SOURCE_ENUM, create_type=False),
            server_default="SCANNER",
            nullable=False,
        ),
        sa.Column(
            "bucket",
            postgresql.ENUM("GAINER", "LOSER", "NEUTRAL", name=BUCKET_ENUM, create_type=False),
        ...
        sa.Column(
            "status",
            postgresql.ENUM("CANDIDATE", "APPROVED", "IGNORED", "DISABLED", name=STATUS_ENUM, create_type=False),
